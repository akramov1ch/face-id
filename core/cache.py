...
class CacheManager:
    def __init__(self):
        try:
            self.redis = redis.Redis(
                ...
                db=0, 
                decode_responses=True, # Bayt emas, string qaytarishi uchun
                socket_timeout=2
            )
        ...
        self.TTL = 3600  # Keshda saqlash vaqti: 1 soat (sekundlarda)

    def get_device_info(self, ip: str):
        """Qurilma va uning filiali haqida ma'lumotni keshdan olish"""
    ...
    def set_device_info(self, device: Device, branch: Branch):
        """Qurilma ma'lumotlarini keshga yozish"""
    ...
    def get_employee_info(self, emp_id: str):
        """Xodim ma'lumotlarini keshdan olish"""
        if not self.redis: return None
        try:
            key = f"emp:{emp_id}"
            data = self.redis.get(key)
